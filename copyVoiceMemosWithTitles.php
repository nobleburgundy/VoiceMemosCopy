...
class CopyVoiceMemos {
    ...
    function run() {
        ...
        foreach ($titles as $k => $v) {
            ...
            preg_match('/\d\d\d\d\d\d\d\d/', $filePath, $matches);
            $date        = reset($matches);
            $name        = $name . "_" . $date;
            ...
            if (file_exists($newFullPath)) {
                ...
                if (strpos($filePath, 'Voice Memos') > -1) {
    ...
    function verifyCopy() {
        ...
        foreach ($oldFileArray as $file) {
            foreach ($newFileArray as $newFile) {
                if (strpos($newFile, $file) > -1) {
                    $foundCount++;
                    break;
                }
            }
        ...
        if ($foundCount = count($oldFileArray)) {
            $pass = true;
        }

        $result = $pass ? 'PASS - all VoiceMemo files found' :
            'FAIL - there are some missing VoiceMemo files. Check the origin.';
